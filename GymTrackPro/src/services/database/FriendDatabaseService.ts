...
import {validateFriendRequest} from '../../utils/sanitize';
  ...
  async sendFriendRequest(
    ...
    return this.executeOperation(
      async () => {
        ...
        // Convert to the format expected by the FriendRequest interface
        const friendRequestForDb = {
          id: '', // Will be filled when added to Firestore
          senderId: fromUserId,
          senderName: username,
  ...
  async acceptFriendRequest(requestId: string, userId: string, isOnline: boolean): Promise<ApiResponse<boolean>> {
    return this.executeOperation(
      async () => {
        ...
        const fromUserFriend: Partial<Friend> = {
        ...
        const toUserFriend: Partial<Friend> = {
          userId: userId,
